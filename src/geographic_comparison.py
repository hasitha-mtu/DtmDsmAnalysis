...
def compare_dsm_with_geographic_overlap(webodm_file, bluesky_file, 
    ...
    if abs(mean_diff) < 0.30 and rmse < 0.50:
        print(f"   ✅ Good agreement! No major systematic offset")
        print(f"   ✅ RMSE {rmse:.2f}m is reasonable for {comparison_scale}m scale comparison")
        print(f"   ✅ Safe to proceed with WebODM DSM")
    elif abs(mean_diff) < 0.60 and rmse < 1.0:
        print(f"   ⚠️  Moderate offset: {mean_diff:.2f}m")
        print(f"   ⚠️  Consider checking vertical datum or GCP elevations")
    else:
        print(f"   ❌ Large offset: {mean_diff:.2f}m, RMSE: {rmse:.2f}m")
        print(f"   ❌ Investigate: CRS, datum, GCPs, or processing issues")
